...
type ConnectionCallback<'cb, 'cx> = dyn for<'f> FnMut(&'f Context<'cx, 'cb>, &'f mut Connection<'cb, 'cx>, ConnectionEvent, i32, Option<&'f error::StreamError<'f, 'f>>) + 'cb;
...
type StanzaCallback<'cb, 'cx> = dyn for<'f> FnMut(&'f Context<'cx, 'cb>, &'f mut Connection<'cb, 'cx>, &'f Stanza<'f>) -> bool + 'cb;
...
impl<'cb, 'cx> Connection<'cb, 'cx> {
	...
	pub fn connect_client<CB>(self, alt_host: Option<&str>, alt_port: impl Into<Option<u16>>, handler: CB) -> Result<Context<'cx, 'cb>, error::ConnectError<'cb, 'cx>>
		where
			CB: for<'f> FnMut(&'f Context<'cx, 'cb>, &'f mut Connection<'cb, 'cx>, ConnectionEvent, i32, Option<&'f error::StreamError<'f, 'f>>) + 'cb,
	{
	...
	pub fn id_handler_add<CB>(&mut self, handler: CB, id: impl Into<String>) -> Option<IdHandlerId<'cb, 'cx, CB>>
		where
			CB: for<'f> FnMut(&'f Context<'cx, 'cb>, &'f mut Connection<'cb, 'cx>, &'f Stanza<'f>) -> bool + 'cb,
	{
		let id = id.into();
