...
use crate::{ConnectClientError, ConnectionError, ConnectionFlags, Context, Error, Result, Stanza, StreamError, FFI};
...
impl<'cb, 'cx> Connection<'cb, 'cx> {
	...
	unsafe extern "C" fn connection_handler_cb<CB>(
		...
		CB: FnMut(&Context<'cx, 'cb>, &mut Connection<'cb, 'cx>, ConnectionEvent) + Send + Sync + 'cb,
	{
		let connection_handler = ConnectionFatHandler::from_userdata(userdata);
		if let Some(fat_handlers) = connection_handler.handlers.upgrade() {
			...
			let event = match event {
				...
				sys::xmpp_conn_event_t::XMPP_CONN_DISCONNECT => {
					let stream_error = stream_error.as_ref().map(StreamError::from);
	...
	where
		CB: FnMut(&Context<'cx, 'cb>, &mut Connection<'cb, 'cx>) -> HandlerResult + Send + Sync + 'cb,
	{
		let timed_handler = TimedFatHandler::from_userdata(userdata);
	...
	where
		CB: FnMut(&Context<'cx, 'cb>, &mut Connection<'cb, 'cx>, &Stanza) -> HandlerResult + Send + Sync + 'cb,
	{
		let stanza_handler = StanzaFatHandler::from_userdata(userdata);
	...
	where
		CB: Fn(&Connection<'cb, 'cx>, usize) -> Option<String> + Send + Sync + 'cb,
	{
	...
	#[cfg(feature = "libstrophe-0_12_0")]
	#[inline]
	/// [xmpp_conn_set_sm_state](https://strophe.im/libstrophe/doc/0.13.0/group___connections.html#gace3fa6449c31ce4f5db6ab9f0375eb47)
	...
	where
		CB: FnMut(&mut Connection<'cb, 'cx>, SerializedSmStateRef) + Send + Sync + 'cb,
	{
	...
	where
		L: FnMut(&Context<'cx, 'cb>, &mut Connection<'cb, 'cx>) -> HandlerResult + Send + Sync + 'cb,
		R: FnMut(&Context<'cx, 'cb>, &mut Connection<'cb, 'cx>) -> HandlerResult + Send + Sync + 'cb,
	{
		ptr::eq(
	...
	where
		L: FnMut(&Context<'cx, 'cb>, &mut Connection<'cb, 'cx>, &Stanza) -> HandlerResult + Send + Sync + 'cb,
		R: FnMut(&Context<'cx, 'cb>, &mut Connection<'cb, 'cx>, &Stanza) -> HandlerResult + Send + Sync + 'cb,
	{
	...
	where
		L: FnMut(&Context<'cx, 'cb>, &mut Connection<'cb, 'cx>, ConnectionEvent) + Send + Sync + 'cb,
		R: FnMut(&Context<'cx, 'cb>, &mut Connection<'cb, 'cx>, ConnectionEvent) + Send + Sync + 'cb,
	{
		ptr::eq(
