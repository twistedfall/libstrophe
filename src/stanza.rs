...
use crate::{ALLOC_CONTEXT, Error, ErrorType, FFI, Result, ToTextError};
...
///   * `Send`
///
/// [docs]: https://strophe.im/libstrophe/doc/0.13.0/group___stanza.html
/// [sources]: https://github.com/strophe/libstrophe/blob/0.14.0/src/stanza.c
/// [xmpp_stanza_to_text]: https://strophe.im/libstrophe/doc/0.13.0/group___stanza.html#ga2918484877ac34d483cc14cf5e957fad
/// [xmpp_stanza_release]: https://strophe.im/libstrophe/doc/0.13.0/group___stanza.html#gaa231317e56af0b974d7a28793ebefb83
/// [xmpp_stanza_copy]: https://strophe.im/libstrophe/doc/0.13.0/group___stanza.html#ga1ab1122fdf477d9d755fc094b4fbf6b6
#[derive(Debug, Eq)]
pub struct Stanza {
...
impl Stanza {
	#[inline]
	/// [xmpp_stanza_new](https://strophe.im/libstrophe/doc/0.13.0/group___stanza.html#ga0422973c3e2f6851c1ff1868e476f118)
	///
	/// The newly created stanza is not really useful until you assign an internal type to it. To do
	/// that you must call [Self::set_text()] to make it `XMPP_STANZA_TEXT` stanza or [Self::set_name()] to make
	...
	#[inline]
	/// [xmpp_presence_new](https://strophe.im/libstrophe/doc/0.13.0/group___stanza.html#gac47cc19072d39056d24d579b3a96b5db)
	pub fn new_presence() -> Self {
		unsafe { Stanza::from_owned(sys::xmpp_presence_new(ALLOC_CONTEXT.as_ptr())) }
	}

	#[inline]
	/// [xmpp_iq_new](https://strophe.im/libstrophe/doc/0.13.0/group___stanza.html#gad3fccc89f1367c5014dce97238d75f4d)
	...
	#[inline]
	/// [xmpp_message_new](https://strophe.im/libstrophe/doc/0.13.0/group___stanza.html#ga850783185475f4324423f2af876774eb)
	...
	/// [xmpp_error_new](https://strophe.im/libstrophe/doc/0.13.0/group___stanza.html#ga867086f16735eff5220a116d9d5e353b)
	...
	/// [xmpp_stanza_new_from_string](https://strophe.im/libstrophe/doc/0.13.0/group___stanza.html#ga334bbf723f451d1ca7ffb747029c0b4a)
	...
	/// # Safety
	/// inner must be a valid pointer to a previously allocated [sys::xmpp_stanza_t], and you must make sure
	...
	pub unsafe fn from_owned(inner: *mut sys::xmpp_stanza_t) -> Self {
		unsafe { Stanza::with_inner(inner, true) }
	}
	...
	pub unsafe fn from_ref<'st>(inner: *const sys::xmpp_stanza_t) -> StanzaRef<'st> {
		unsafe { Stanza::with_inner(inner.cast_mut(), false).into() }
	}

	#[inline]
	/// Create a borrowing stanza from the mutable raw pointer
	/// # Safety
	/// inner must be a valid pointer to a previously allocated mutable [sys::xmpp_stanza_t], and you must
	...
	pub unsafe fn from_ref_mut<'st>(inner: *mut sys::xmpp_stanza_t) -> StanzaMutRef<'st> {
		unsafe { Stanza::with_inner(inner, false).into() }
	}
	...
	///
	/// Generally libstrophe's [sys::xmpp_stanza_t] needs [sys::xmpp_ctx_t] only for allocation so it's possible to make [Stanza] 'static
	/// and not dependent on a particular context by using global [ALLOC_CONTEXT]. That's what is done when a new [Stanza] is
	/// created through methods of this struct, but when [Stanza] is copied (through [Self::clone()] or [Self::reply()]) we don't control
	/// the initial context, and it is set by `libstrophe` itself (e.g. in callback of `xmpp_handler_add`). In this case it
	...
	fn set_alloc_context(&mut self) {
		// this is dependent on internal representation of _xmpp_stanza_t in common.h for versions 0.9.3 to 0.14.0 (libstrophe-0_14), update if needed
		#[repr(C)]
		struct XmppStanzaRepr {
			rf: c_int,
		...
		let inner = unsafe { self.inner.as_ptr().cast::<XmppStanzaRepr>().as_mut() }.expect("Null pointer for Stanza context");
	...
	#[inline]
	/// [xmpp_stanza_is_text](https://strophe.im/libstrophe/doc/0.13.0/group___stanza.html#ga3607a9a49c3614b8599b5ec469a65740)
	pub fn is_text(&self) -> bool {
		FFI(unsafe { sys::xmpp_stanza_is_text(self.inner.as_ptr()) }).receive_bool()
	}

	#[inline]
	/// [xmpp_stanza_is_tag](https://strophe.im/libstrophe/doc/0.13.0/group___stanza.html#gae39816bfdd86e97ca2b03c52813a5a12)
	...
	#[inline]
	/// [xmpp_stanza_set_name](https://strophe.im/libstrophe/doc/0.13.0/group___stanza.html#ga8331fbddc0f2fc7286a267ef60c69df2)
	///
	...
	#[inline]
	/// [xmpp_stanza_get_name](https://strophe.im/libstrophe/doc/0.13.0/group___stanza.html#gabaffb70aab506a9f2912edb50c43c172)
	pub fn name(&self) -> Option<&str> {
		unsafe { FFI(sys::xmpp_stanza_get_name(self.inner.as_ptr())).receive() }
	}

	#[inline]
	/// [xmpp_stanza_get_attribute_count](https://strophe.im/libstrophe/doc/0.13.0/group___stanza.html#ga015147125e509724524c0aecb536c59c)
	pub fn attribute_count(&self) -> i32 {
		unsafe { sys::xmpp_stanza_get_attribute_count(self.inner.as_ptr()) }
	}

	#[inline]
	/// [xmpp_stanza_set_attribute](https://strophe.im/libstrophe/doc/0.13.0/group___stanza.html#ga06ab477beba98d2f5b66d54e530bfa2d)
	...
	#[inline]
	/// [xmpp_stanza_get_attribute](https://strophe.im/libstrophe/doc/0.13.0/group___stanza.html#gad836688d17b7c5af148f32823e72b81b)
	...
	/// [xmpp_stanza_get_attributes](https://strophe.im/libstrophe/doc/0.13.0/group___stanza.html#ga2eea8820dcf9b3e2440a06de55a35850)
	///
	...
	pub fn attributes(&self) -> HashMap<&str, &str> {
		...
		let count_usize = usize::try_from(count).unwrap_or(usize::MIN);
		let mut out = HashMap::with_capacity(count_usize);
		let mut arr = vec![ptr::null::<c_char>(); count_usize * 2];
		unsafe {
	...
	#[inline]
	/// [xmpp_stanza_del_attribute](https://strophe.im/libstrophe/doc/0.13.0/group___stanza.html#gae335c3ea4b5517d2e4cdfdc5cc41e143)
	...
	#[inline]
	/// [xmpp_stanza_set_text_with_size](https://strophe.im/libstrophe/doc/0.13.0/group___stanza.html#ga779812852611947d0181cd4c58644ef1)
	///
	...
	pub fn set_text(&mut self, text: impl AsRef<str>) -> Result<()> {
		...
		unsafe { sys::xmpp_stanza_set_text_with_size(self.inner.as_mut(), text.as_ptr().cast::<c_char>(), text.len()) }
			.into_result()
	}

	#[inline]
	/// [xmpp_stanza_get_text](https://strophe.im/libstrophe/doc/0.13.0/group___stanza.html#ga95476dfe39bb7acd8e6c534ef3877602)
	pub fn text(&self) -> Option<String> {
		unsafe { FFI(sys::xmpp_stanza_get_text(self.inner.as_ptr())).receive_with_free(|x| ALLOC_CONTEXT.free(x)) }
	}

	#[inline]
	/// [xmpp_stanza_set_id](https://strophe.im/libstrophe/doc/0.13.0/group___stanza.html#gaa19a4d40d3383881b3266631dd9f2a0d)
	...
	#[inline]
	/// [xmpp_stanza_get_id](https://strophe.im/libstrophe/doc/0.13.0/group___stanza.html#ga7aebb4b618deb9fc24b9dae5418a0267)
	pub fn id(&self) -> Option<&str> {
		unsafe { FFI(sys::xmpp_stanza_get_id(self.inner.as_ptr())).receive() }
	}

	#[inline]
	/// [xmpp_stanza_set_ns](https://strophe.im/libstrophe/doc/0.13.0/group___stanza.html#ga2e55fd5671aa9803959ec19518a9adcf)
	...
	#[inline]
	/// [xmpp_stanza_get_ns](https://strophe.im/libstrophe/doc/0.13.0/group___stanza.html#gabea315ad8a9a924489701ec5496ce4a7)
	pub fn ns(&self) -> Option<&str> {
		unsafe { FFI(sys::xmpp_stanza_get_ns(self.inner.as_ptr())).receive() }
	}

	#[inline]
	/// [xmpp_stanza_set_type](https://strophe.im/libstrophe/doc/0.13.0/group___stanza.html#ga30d9a7a46ec52c8c8675d31a6af1273b)
	...
	#[inline]
	/// [xmpp_stanza_get_type](https://strophe.im/libstrophe/doc/0.13.0/group___stanza.html#ga017c704661831141826ed8b4d3daba75)
	pub fn stanza_type(&self) -> Option<&str> {
		unsafe { FFI(sys::xmpp_stanza_get_type(self.inner.as_ptr())).receive() }
	}

	#[inline]
	/// [xmpp_stanza_set_to](https://strophe.im/libstrophe/doc/0.13.0/group___stanza.html#ga095ac729f5b65795cae689f7462a0a89)
	...
	#[inline]
	/// [xmpp_stanza_get_to](https://strophe.im/libstrophe/doc/0.13.0/group___stanza.html#gabb04ada2be97b6d9440b28971dc5872d)
	pub fn to(&self) -> Option<&str> {
		unsafe { FFI(sys::xmpp_stanza_get_to(self.inner.as_ptr())).receive() }
	}

	#[inline]
	/// [xmpp_stanza_set_from](https://strophe.im/libstrophe/doc/0.13.0/group___stanza.html#ga368fd01efb2aa6ad0ec2c36233d2c551)
	...
	#[inline]
	/// [xmpp_stanza_get_from](https://strophe.im/libstrophe/doc/0.13.0/group___stanza.html#ga03f3a2873592536e69c16905edbcd228)
	...
	/// [xmpp_stanza_get_child_by_path](https://strophe.im/libstrophe/doc/0.13.0/group___stanza.html#ga12567a82abab6a54c396ea56cb895981)
	///
	...
	#[inline]
	/// [xmpp_stanza_add_child](https://strophe.im/libstrophe/doc/0.13.0/group___stanza.html#ga9cfdeabcfc45409d2dfff4b364f84a0c)
	pub fn add_child(&mut self, mut child: Stanza) -> Result<()> {
		unsafe { sys::xmpp_stanza_add_child(self.inner.as_mut(), child.inner.as_mut()) }.into_result()
	}

	#[inline]
	/// [xmpp_stanza_reply](https://strophe.im/libstrophe/doc/0.13.0/group___stanza.html#ga32c20758b86bf9c46688e58878a284b5)
	...
	#[inline]
	#[cfg(feature = "libstrophe-0_10_0")]
	/// [xmpp_stanza_reply_error](https://strophe.im/libstrophe/doc/0.13.0/group___stanza.html#ga62a222d584f3890b957ab507070664ff)
	...
	#[inline]
	/// [xmpp_message_set_body](https://strophe.im/libstrophe/doc/0.13.0/group___stanza.html#gace4a07d21a6700692d22ea13200d13f5)
	...
	#[inline]
	/// [xmpp_message_get_body](https://strophe.im/libstrophe/doc/0.13.0/group___stanza.html#ga6659d4692fdddc6758f99c1f867fa3e2)
...
/// [XMPP_STANZA_NAME_IN_NS](https://strophe.im/libstrophe/doc/0.13.0/strophe_8h.html#a3c83ba7062a2099e61d44c9226bdb286)
	...
	E: From<Error>,
{
	...
	res.into_result().map_err(E::from).and_then(|_| {
		let text = unsafe { CStr::from_bytes_with_nul_unchecked(slice::from_raw_parts(buf.cast::<u8>(), buflen + 1)) };
		cb(text)
	})
}
