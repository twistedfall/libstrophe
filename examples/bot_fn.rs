//! Port of the https://github.com/strophe/libstrophe/blob/0.14.0/examples/bot.c

use libstrophe::{Connection, ConnectionEvent, Context, HandlerResult, Stanza};

fn version_handler(_ctx: &Context, conn: &mut Connection, stanza: &Stanza) -> HandlerResult {
...
fn message_handler(_ctx: &Context, conn: &mut Connection, stanza: &Stanza) -> HandlerResult {
	...
	match stanza.stanza_type() {
		Some("error") | None => return HandlerResult::KeepHandler,
		_ => (),
	}
